...
function App() {
  ...
  const [books, updateBooks] = useState(defaultBooks);
            ...
            render={() => (
              <>
                ...
                {Object.keys(bookShelves).map((key) => (
                  <BookShelf
                    key={bookShelves[key]}
                    status={bookShelves[key]}
                    books={books.filter(book => book.status === bookShelves[key])} // filter books so that only books with a status that matches the current shelf are added
                  />
                ))}
              </>
            )}
...
class OldBooksApp extends React.Component {
  state = {
    /**
